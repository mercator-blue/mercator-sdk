...
- `@mercator-blue/sdk/colormaps` no longer re-exports the `Colormap`,
  `ColormapStop`, and `ColormapSpec` types. They were a convenience re-export
  with no internal consumer and required ugly alias plumbing to keep
  documented. `ColormapSpec` is still exported from the main `@mercator-blue/sdk`
  entry. The runtime exports here (`PALETTES`, `resolveColormap`,
  `COLORMAP_SIZE`, `sampleColormapCss`) are unchanged.
